...
Payment, discovery, distribution, fulfillment, and the sellable offer are separate decisions. The current preflight engine uses GitHub Pages for the free route, a human checkout for the full audit, and x402 only for a future metered route.
...
The buyer may be software or another agent and the unit is one machine-readable decision report. x402 is the proposed rail only after three qualified recurring-use requests, one repeated agent workflow, an approved runtime host, and a dedicated receiving wallet. Until then, the functional public route remains free.

## Why x402 Is Not The Business

The live Bazaar shows repeated payment for concrete machine utilities, but resource-level payer counters are not audited revenue and may double-count users across resources. The current product therefore sells a specific preflight outcome rather than generic reasoning or implementation information. x402 remains a rail and discovery mechanism, not the company identity.
